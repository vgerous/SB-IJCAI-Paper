...
def forward_single_np(Q, p, G, h, A, b):
    ...
    """
    the following are local-alternation: enforce non-negativity for all decision variables
    """
    additionalCon = z_ >= 0

    cons = [x for x in [eqCon, ineqCon, slacksCon, additionalCon] if x is not None]
    prob = cp.Problem(obj, cons)
    prob.solve(cp.GUROBI)  # solver=cp.SCS, max_iters=5000, verbose=False)
