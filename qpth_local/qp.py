...
def QPFunction(eps=1e-12, verbose=0, notImprovedLim=3,
                 ...
                 check_Q_spd=True, nn_device=torch.device('cpu')):
    class QPFunctionFn(Function):
        ...
        def forward(ctx, Q_, p_, G_, h_, A_, b_):
            ...
            if solver == QPSolvers.PDIPM_BATCHED:
                ...
                vals = torch.Tensor(nBatch).to(nn_device).type_as(Q)
                zhats = torch.Tensor(nBatch, ctx.nz).to(nn_device).type_as(Q)
                lams = torch.Tensor(nBatch, ctx.nineq).to(nn_device).type_as(Q)
                nus = torch.Tensor(nBatch, ctx.neq).to(nn_device).type_as(Q) \
                    if ctx.neq > 0 else torch.Tensor()
                slacks = torch.Tensor(nBatch, ctx.nineq).to(nn_device).type_as(Q)
                for i in range(nBatch):
                    ...
                    zhats[i] = torch.Tensor(zhati).to(nn_device)
                    lams[i] = torch.Tensor(lami).to(nn_device)
                    slacks[i] = torch.Tensor(si).to(nn_device)
                    if neq > 0:
                        nus[i] = torch.Tensor(nui).to(nn_device)
